...
if($_SERVER['SERVER_PORT'] != '443') {
    header('Location: https://lider-finance.ru/news.php');
}
    ...
    <div class="prewie">
      ...
      <div class="row-fluid block">
		<br /><p><h3>Новости</h3></p>
          ...
          <?php
          $request = mysql_query("SELECT * FROM news", $db);
          $i = 1;
          while($news = mysql_fetch_assoc($request)){
              $text = $news['text'];
              $day = date("d" ,$news['date']);
              $month = date("m" ,$news['date']);
              $year = date("Y" ,$news['date']);
              echo "<tr><td>$i</td><td>".$day."/".$month."/".$year."</td><td>".$text."</td></tr>";
              $i++;
          }
          echo <<<HERE
			</tbody>
