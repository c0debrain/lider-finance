...
if($_SERVER['SERVER_PORT'] != '443') {
    header('Location: https://lider-finance.ru/investor/deposits/the-fast-start/index.php');
}
if(empty($_SESSION['login']) AND empty($_SESSION['id'])) {
    header('Location: index.php');
}
if(!empty($_POST['PAYMENT_AMOUNT'])){
    if($_POST['PAYMENT_AMOUNT'] < 1 || $_POST['PAYMENT_AMOUNT'] > 300){
        header('Location: index.php');
    }
}
        ...
        <div class="row">
        	<div class="col-lg-12">
        		<ul class="breadcrumb">
				  ...
				  <li class="active">Вклад: "The Fast Start"</li>
				</ul>
        	</div>
        </div>
		<div class="row">
        	<div class="col-lg-3 text-center">
        	 <ul class="list-group">
				  <a href="#" class="list-group-item active">
				    <h4>Вклад: The Fast Start</h4>
				</a>
				  <li class="list-group-item">
				  	114% дохода за вклад на 60 дней
				  </li>
				  <li class="list-group-item">
				  	Инвестиционный лимит 25 — 300 USD
				  </li>
				  <li class="list-group-item">
				  	Начисления на внутренний счет  1 раз в сутки <br>(на общую сумму 1.9% от суммы вклада).
				  </li>
			 </ul>
			</div>
			<div class="col-lg-6">
				<fieldset>
        			...
        			<p>Открытие счета производится одновременно с взносом денежных средств. Данный вклад ограничивается лимитом взноса средств.(25 — 300 USD)</p>
				    <form class="form-horizontal" action='https://perfectmoney.com/api/step1.asp' method='POST' onkeypress="if(event.keyCode == 13) return false;">
					    <div class="control-group">
						    ...
						    <div class="controls">
						    	<div class="input-group" style="max-width: 200px;">
								  ...
								  <input id="OutNumber" type="number" name='PAYMENT_AMOUNT' class="form-control" onchange="if ((this.value >= 25) && (this.value <= 300)) {getElementById('OutDeposit').disabled = false;}else{getElementById('OutDeposit').disabled = true;}"">
								  <span class="input-group-addon">.00</span>
								</div>
							    <p class="help-block">Сумма взноса должна быть в диапозне от 25-300 USD.</p>
						    </div>
					    </div>
					    <input type='hidden' name='PAYEE_ACCOUNT' value='U1875378'/>
				        <input type='hidden' name='PAYEE_NAME' value='Lider finance'/>
				        <input type='hidden' name='PAYMENT_ID' value='<?php echo $id."-thefaststart"; ?>'/>
				        ...
				        <input type='hidden' name='SUGGESTED_MEMO' value='Взнос средств на открытие вклада The Fast Start.'/>
